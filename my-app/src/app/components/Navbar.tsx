...
const Navbar: React.FC = () => {
  ...
  const [balance, setBalance] = useState<string>('0 SS'); // Set initial balance to '0 AA'
  const logoRef = useRef<HTMLDivElement | null>(null); // Ref for logo animation
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        setBalance('15 SS'); // Replace this with your logic to fetch balance
      } catch (error) {
